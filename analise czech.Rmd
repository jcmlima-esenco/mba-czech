...
```{r}
model_ambos <- glm(inadimplentes ~ durantion + amount, data = loan)
summary(model_ambos)
melhor_modelo <- model_ambos
```
...
```{r}
model_candidato <- glm(inadimplentes ~ amount + durantion + payments, data = melhor_modelo)
summary(model_candidato)
```
Podemos visualizar que a duração não influencia ( p-value = 0.146 )
...
# Sexo influencia no perfil do inadimplente?

A distribuição de todos os clientes, na base é

```{r}

client %>% ggplot(aes(x=sex)) + geom_bar() + theme_bw()


```

## Avaliando apenas entre os clientes inadimplentes sejam eles com contrato terminado ou não, existe alguma correlação?

```{r message=TRUE, warning=TRUE, paged.print=TRUE}
account_loan <- merge(account_client, loan, by="account_id")
loan_default <- account_loan %>% mutate(inadimplentes = (status == "B" | status == "D" ))
loan_default <- loan_default %>% mutate(is_male = (sex == "Male")) ## dummy
model_loan_client <- glm(inadimplentes ~ amount + is_male, data = loan_default)
summary(model_loan_client)
```

Não é relevante o sexo do cliente.
